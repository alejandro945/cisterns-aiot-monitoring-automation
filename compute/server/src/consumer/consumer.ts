...
export class Consumer implements OnModuleInit {
    ...
    constructor(private readonly consumerService: ConsumerService, private readonly databaseSerive: DatabaseService,
    ...
    async onModuleInit() {
        ...
        await this.consumerService.consume({
            ...
            onMessage: async (message: KafkaMessage, topic: string) => {
                ...
                Logger.log(`Processing message from ${hostname} in topic ${subtopic} with the value of: ${message.value.toString()}`);
     ...
     */
    private async handleConsumerLogic(message: KafkaMessage, hostname: string, subtopic: string) {
        switch (subtopic) {
            case SUBTOPIC.IP:
                const ip = message.value.toString();
                this.databaseSerive.getDbHandle().collection('devices').updateOne(
                    { hostname },
                    { $set: { ip, status: true } },
                    { upsert: true }
                );
            ...
            case SUBTOPIC.FREEMEMORY:
                ...
                if (freeMemory < minHeapSize) {
                    this.databaseSerive.getDbHandle().collection('alerts').insertOne({
                        hostname,
            ...
            case SUBTOPIC.JSON:
                ...
                const maxConsumption = parseInt(this.configService.get('MAX_CONSUMPTION'));
                //Just insert the measurment in the database if the previous value is not the same or is greater
                const lastValue = await this.databaseSerive.getDbHandle().collection('measurments').findOne({
                    hostname,
                }, { sort: { createdAt: -1 } });
                if (!lastValue || lastValue.value <= json.value) {
                    this.databaseSerive.getDbHandle().collection('measurments').insertOne({
                        hostname,
                        value: json.value,
                        createdAt: new Date(),
                    });
                } else {
                    Logger.log(`Value ${json.value} is less than the last value ${lastValue.value}`);
                }
                //Get the first value registered on this day in the measurments colleciton base on the hostname and created at date
                const firstValueOfTheDay = await this.databaseSerive.getDbHandle().collection('measurments').findOne({
                    hostname,
